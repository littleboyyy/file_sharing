...
int main (int argc, char **argv){
  ...
  fwin->app = gtk_application_new ("com.gtk.myapp", G_APPLICATION_FLAGS_NONE);
...
void check_login (GtkWidget *button, Window *win)
{
	...
	bytes_recv = recv(client_sock, &msg, sizeof(message), 0);
	if (bytes_recv < 0)
		perror("\nError: ");
	...
	//new window
    	...
    	label = gtk_label_new("Username or Password is incorrect.");
...
void show_members(GtkWidget *button,Window *win) 
{
	...
	for(i=0;i<n;i++){
		...
		//g_signal_connect_swapped(delete[i], "clicked", G_CALLBACK(gtk_window_destroy), subwindow);
		//wind[i]->window
