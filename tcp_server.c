...
int plogin(int sockfd, MYSQL *conn, message msg) {
	...
	while(1){
		...
		row = mysql_fetch_row(res);//lay du lieu hang dau tien luu vao row
		strcpy(uID,row[2]);
		
		//check password
		if(strcmp(msg.payld[1],row[1])==0){		
			msg_send.code = LOGIN_SUCC;					
		}		
		else msg_send.code=PWD_INCORR;
		}else msg_send.code=UN_INCORR;

		if(send(sockfd, &msg_send, sizeof(msg_send), 0) < 0) return 0;
		mysql_free_result(res);
		if(msg_send.code==LOGIN_SUCC) break;
	}
	return 1;
}
